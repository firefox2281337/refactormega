...
"""
Сервис для управления проверкой кампаний.
Отвечает за обработку файлов кампаний и управление задачами.
"""
...
class CampaignsService:
    ...
    def validate_file(self, file) -> Tuple[bool, str]:
        ...
        if not file.filename.startswith('cgr'):
            return False, 'Файл должен начинаться с "cgr"'
    ...
    def process_file(self, cgr_file, client_ip: str = None) -> Tuple[bool, str]:
        """
        ...
        # Валидируем файл
        valid, error_msg = self.validate_file(cgr_file)
        if not valid:
            return False, error_msg
        
        # Логируем начало обработки
        log_user_access(
            page="Campaigns Processing",
            client_ip=client_ip or "Unknown",
            current_time=datetime.now().strftime("%Y.%m.%d %H:%M:%S"),
            message=f"Начало обработки файла кампаний: {cgr_file.filename}"
        )
        
        # Сохраняем файл временно
        cgr_filename = self._create_safe_filename(cgr_file.filename)
        cgr_path = self.TEMP_DIR / cgr_filename
        
        try:
            cgr_file.save(cgr_path)
            print(f"Файл сохранен: {cgr_file.filename} -> {cgr_path}")
        except Exception as e:
        ...
        thread = threading.Thread(
            ...
            args=(cgr_path,),
            name=f"CampaignsProcessing-{self.current_task.task_id}"
        )
        thread.daemon = True
        thread.start()
    ...
    def _run_processing(self, cgr_path: Path):
        ...
        try:
            ...
            self._cleanup_temp_files(cgr_path)
