# web/utils/nexus_config.py
"""
Конфигурация страниц Nexus.
Централизованное хранение всех конфигураций интерфейса и настроек модулей.
"""

from typing import Dict, List, Any, Optional

# Базовая конфигурация кнопок навигации
BUTTON_CONFIGS: Dict[str, List[Dict[str, Any]]] = {
    'main': [
        {
...
AUTODAILYES_CONFIGS: Dict[str, Dict[str, Any]] = {
    'autodeals': {
...
AUTOREG_CONFIGS: Dict[str, Dict[str, Any]] = {
    'ipoteka': {
    ...
    'dvr': {
        ...
        'config': {
            ...
            'templateHeaders': [
                ...
                "Другой полис", "Дата начала страхования/дата заключения", 
                "Кредитный договор Дата", "Прошлый период Страховая сумма", "Марка", "Модель", "Год выпуска", "СтраховаяСумма", "Риск", "ТипСобытия", "Стадия"
            ]
        }
    }
}
...
class NexusConfigManager:
    """Менеджер конфигураций Nexus"""
    
    @staticmethod
    def get_page_config(page_type: str) -> Optional[Dict[str, Any]]:
        """Получение конфигурации страницы"""
        return PAGE_CONFIGS.get(page_type)
    
    @staticmethod
    def get_button_config(button_type: str) -> Optional[List[Dict[str, Any]]]:
        """Получение конфигурации кнопок"""
        return BUTTON_CONFIGS.get(button_type)
    
    @staticmethod
    def get_autodailyes_config(module_type: str) -> Optional[Dict[str, Any]]:
        """Получение конфигурации автоежедневок"""
        return AUTODAILYES_CONFIGS.get(module_type)
    
    @staticmethod
    def get_autoreg_config(register_type: str) -> Optional[Dict[str, Any]]:
        """Получение конфигурации автореестров"""
        return AUTOREG_CONFIGS.get(register_type)
    
    @staticmethod
    def get_all_register_types() -> List[str]:
        """Получение списка всех типов реестров"""
        return list(AUTOREG_CONFIGS.keys())
    
    @staticmethod
    def get_all_autodailyes_types() -> List[str]:
        """Получение списка всех типов автоежедневок"""
        return list(AUTODAILYES_CONFIGS.keys())
    
    @staticmethod
    def validate_register_type(register_type: str) -> bool:
        """Проверка существования типа реестра"""
        return register_type in AUTOREG_CONFIGS
    
    @staticmethod
    def validate_autodailyes_type(module_type: str) -> bool:
        """Проверка существования типа автоежедневок"""
        return module_type in AUTODAILYES_CONFIGS


# Создаем глобальный экземпляр менеджера конфигураций
nexus_config_manager = NexusConfigManager()
