...
class DataService:
    ...
    def get_kasko_prolongation_data(
        self, 
        start_date: Optional[str] = None, 
        end_date: Optional[str] = None,
        insurance_type: Optional[List[str]] = None, 
        channel: Optional[str] = None, 
        branch_code: Optional[List[str]] = None
        ...
        try:
            ...
            period_start = self._format_date_period(start_date)
            period_end = self._format_date_period(end_date)
            
            where_clause = self._build_where_clause(
                period_start, period_end, insurance_type, channel, branch_code
            )
    ...
    def execute_sql_query(self, contracts_string: str, query_type: str = "contacts") -> Optional[List[Dict]]:
        """
        ...
        """
        try:
            query = self._get_query_by_type(contracts_string, query_type)
    ...
    def process_metragi_file(self, file) -> List[Tuple[str, bytes]]:
        """
        ...
        try:
            ...
            leads = self._add_base_columns(leads)
            
            # Генерируем ID
            leads = self._generate_ids(leads)
            
            # Устанавливаем даты
            leads = self._set_dates(leads)
            
            # Обрабатываем связанные данные
            contacts = self._process_contacts_data(leads)
            registration = self._process_registration_data(contacts)
            passport = self._process_passport_data(contacts)
            ipoteka = self._create_ipoteka_data(leads)
            
            # Очищаем основные данные
            leads = self._clean_leads_data(leads)
            
            # Создаем файлы для скачивания
            return self._create_download_files(contacts, leads, ipoteka, passport, registration)
            
        except Exception as e:
            log_error(f"Ошибка обработки метражей: {str(e)}")
            raise
    
    def get_gender_from_patronymic(self, name: str) -> str:
        """Определяет пол по отчеству"""
        male_endings = [
            "ович", "евич", "ич", "ов", "ев", "ин", "ский", "овичи", "евичи", "ины", 
            "яты", "ук", "ец", "ей", "инский", "еев", "овец", "ыш", "овичев", "ий", 
            "ын", "уй", "их", "ек", "кевич", "сов", "аров", "зов", "итов", "ров", 
            "овин", "инов", "я", "горов", "ор", "ус", "чук", "ый", "иненко", 
            "ешкин", "енко", "овкин", "дьев", "енков", "ак", "ес", "оглы", "сон", "вар", "волод"
        ]
        
        female_endings = [
            "овна", "евна", "ична", "ина", "ская", "ых", "ова", "ева", "ята", "ук", 
            "ий", "ей", "их", "чук", "ек", "итова", "ас", "инская", "анна", "кова", 
            "ян", "анова", "енко", "олина", "ая", "ю", "чевна", "очина", "ли", 
            "унова", "цова", "лёвна", "кызы", "вна", "ушка", "а", "овина", "еевна"
        ]
    ...
    def _format_date_period(self, date_input: Optional[str]) -> str:
        """Форматирует дату в период YYYY_MM"""
        if isinstance(date_input, str):
            date_object = datetime.strptime(date_input, '%Y-%m-%d')
            return date_object.strftime('%Y_%m')
        else:
            return date_input or datetime.now().strftime('%Y_%m')
    
    def _build_where_clause(
        self, 
        period_start: str, 
        period_end: str,
        insurance_type: Optional[List[str]], 
        channel: Optional[str], 
        branch_code: Optional[List[str]]
    ) -> str:
    ...
    def _get_query_by_type(self, contracts_string: str, query_type: str) -> str:
        """Возвращает SQL запрос по типу"""
        if query_type == "contacts":
            from web.sql_models import get_contacts_sql
            return get_contacts_sql(contracts_string)
        elif query_type == "registration":
            return f"""SELECT * FROM public._subject_master_fl_address 
                      WHERE subject_master_id IN {contracts_string};"""
        elif query_type == "passport":
            return f"""SELECT subject_master_id, passport, passport_issue_org,
    ...
    def _add_base_columns(self, leads: pl.DataFrame) -> pl.DataFrame:
        """Добавляет базовые колонки к данным"""
        return leads.with_columns([
    ...
    def _generate_ids(self, leads: pl.DataFrame) -> pl.DataFrame:
    ...
    def _set_dates(self, leads: pl.DataFrame) -> pl.DataFrame:
        """Устанавливает даты"""
        today = date.today()
    ...
    def _process_contacts_data(self, leads: pl.DataFrame) -> pl.DataFrame:
        ...
        return self._finalize_contacts_data(contacts)
    
    def _finalize_contacts_data(self, contacts: pl.DataFrame) -> pl.DataFrame:
    ...
    def _process_registration_data(self, contacts: pl.DataFrame) -> pl.DataFrame:
    ...
    def _process_passport_data(self, contacts: pl.DataFrame) -> pl.DataFrame:
    ...
    def _create_ipoteka_data(self, leads: pl.DataFrame) -> pl.DataFrame:
        """Создает данные ипотеки"""
        ipo = leads.select("ID_внешней системы")
        return ipo.with_columns([
    ...
    def _clean_leads_data(self, leads: pl.DataFrame) -> pl.DataFrame:
    ...
    def _create_download_files(
        ...
        leads: pl.DataFrame, 
        ipoteka: pl.DataFrame, 
        passport: pl.DataFrame, 
        registration: pl.DataFrame
    ) -> List[Tuple[str, bytes]]:
        """Создает файлы для скачивания"""
        download_files = []
        ...
        registration.write_excel(mem_adress)
        mem_adress.seek(0)
